...
print(f"\nSummary:")
print(f"Total ZIP codes with data: {zipcode_summary.shape[0]}")
print(f"ZIP codes in shapefile: {len(nyc_zipcodes)}")
print(f"ZIP codes plotted: {len(nyc_map)}")
print(f"Cost range: ${nyc_map['NormalizedCost'].min():.0f} - ${nyc_map['NormalizedCost'].max():.0f}")
...
print("\nTop 10 Variables (excluding thresholds):")
print(top10_table)
...
print("\nSummary with variables saved to 'ordinal_logit_summary_header.png'")

# Step 10: Everything saved as PNG images
print("Top 10 coefficients saved to 'ordinal_logit_top10_coefficients.png'")
print("Summary header saved to 'ordinal_logit_summary_header.png'")

# Step 11: Optional - Display full summary
print("\nFull Summary:")
print(res.summary())
...
for name in models:
    ...
    print(f'{name} best AUC on training CV: {search.best_score_:.4f}')
...
print("="*60)
print(r_results_df.to_string(index=False))
print("="*60)
...
print("\n" + "="*60)
print(" LIGHTGBM MODEL PERFORMANCE BREAKDOWN")
print("="*60)
...
for i, class_name in enumerate(class_labels):
    ...
    print(f"{class_name} Accidents:")
    print(f"  Precision: {precision:.3f}")
    print(f"  Recall:    {recall:.3f}")
    print(f"  F1-Score:  {f1_score:.3f}")
    print(f"  Support:   {cm_r[i, :].sum()}")
    print()

# Overall accuracy
overall_accuracy = np.trace(cm_r) / np.sum(cm_r)
print(f"Overall Accuracy: {overall_accuracy:.3f}")

# Macro average recall
macro_recall = np.mean([cm_r[i, i] / cm_r[i, :].sum() for i in range(3)])
print(f"Macro Average Recall: {macro_recall:.3f}")

# Macro average precision
macro_precision = np.mean([cm_r[i, i] / cm_r[:, i].sum() for i in range(3)])
